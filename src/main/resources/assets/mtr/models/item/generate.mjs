...
async function write(speed) {
	return Promise.all([
		fs.writeFile(`rail_connector_${speed}.json`, JSON.stringify({
			...
			"textures": {
				"layer0": `mtr:item/rail_connector_${speed}`
			},
			"overrides": [
				{
					"predicate": {
					...
					"model": `mtr:item/rail_connector_${speed}_selected`
				}
			]
		}, null, 4)),
		fs.writeFile(`rail_connector_${speed}_oneway.json`, JSON.stringify({
			"parent": "item/generated",
			"textures": {
				"layer0": `mtr:item/rail_connector_${speed}_oneway`
			},
			"overrides": [
				{
					"predicate": {
					...
					"model": `mtr:item/rail_connector_${speed}_oneway_selected`
				}
			]
		}, null, 4)),
